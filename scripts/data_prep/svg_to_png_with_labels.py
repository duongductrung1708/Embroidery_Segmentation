...
def create_label_mask(svg_path: str, width: int, height: int, metadata: Dict[str, str]) -> np.ndarray:
    ...
    for child in root.iter():
        ...
        if tag == "path":
            ...
            # Use alpha channel: alpha > 0 indicates path pixels
            alpha_channel = img_array[:, :, 3]
            mask[alpha_channel > 0] = label_value
...
def process_svg_folder(svg_dir: str, output_img_dir: str, output_mask_dir: str, 
    ...
    for svg_file in tqdm(svg_files, desc="Converting SVG to PNG"):
        try:
            # Parse metadata
            _, metadata = parse_svg_with_metadata(str(svg_file))
            ...
            render_svg_to_png(str(svg_file), img_output, img_width, img_height)
...
if __name__ == "__main__":
    ...
    process_svg_folder(svg_dir, output_img_dir, output_mask_dir, args.width, args.height)
